...
const SkillCard = ({ title, icon, skills }: SkillCardProps) => {
  ...
  return (
    <div 
      className={`perspective w-full h-64 cursor-pointer ${isFlipped ? 'animate-flip' : 'animate-flip-back'}`}
      onClick={() => setIsFlipped(!isFlipped)}
    >
      <div className="skill-card w-full h-full relative preserve-3d">
        <div className={`skill-card-front w-full h-full glass p-6 rounded-xl border border-pink/20 flex flex-col items-center justify-center ${isFlipped ? 'hidden' : 'block'}`}>
        ...
        <div className={`skill-card-back w-full h-full glass p-6 rounded-xl border border-pink/20 flex flex-col ${isFlipped ? 'block' : 'hidden'}`}>
...
const SkillsSection = () => {
  ...
  return (
    <section id="skills" className="py-20 bg-gradient-to-b from-dark to-dark/95 relative">
